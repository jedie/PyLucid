...
echo
echo "(1) use SVN only (checkout git repository via github svn gateway)"
echo "(2) use git and clone with readonly"
echo "(3) use git and clone writeable (Needs github pubkey auth!)"
echo
...
case $CHOICE in
    1)  REQUIREMENTS=scripts/requirements/own_projects_svn.txt
        ;;
    2)  REQUIREMENTS=scripts/requirements/own_projects_git_readonly.txt
        ;;
    3)  REQUIREMENTS=scripts/requirements/own_projects_git_writeable.txt
        ;;
