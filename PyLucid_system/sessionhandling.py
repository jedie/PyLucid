...
class sessionhandler:
    ...
    """
    def __init__ ( self, db_cursor, sql_tablename, log_fileobj, timeout_sec=1800, CookieName="session_id" ):
    ...
    def detectSession( self ):
        ...
        if len( cookie_id ) != 32:
            ...
            self.deleteCookie()
            self.log.write( "error;wrong Cookie len!" )
            return
    ...
    def read_session_data( self, cookie_id ):
        ...
        DB_session_id, DB_timestamp, DB_client_IP, \
        DB_client_domain_name, DB_session_data = self.read_from_DB( cookie_id )

        current_IP, current_domain_name = self.get_client_info()
        if not (DB_client_IP == current_IP) and (DB_client_domain_name == current_domain_name):
            self.delete_session()
            raise IndexError, "Wrong client IP / domain name!"

        # Session ist OK

        self.ID                 = DB_session_id
        self.client_IP          = current_IP
        self.client_domain_name = current_domain_name
        self.session_data       = DB_session_data
    ...
    def read_from_DB( self, session_id ):
        ...
        DB_session_id          = DB_data[0]
        DB_timestamp           = DB_data[1]
        DB_client_IP           = DB_data[2]
        DB_client_domain_name  = DB_data[3]
        DB_session_data        = DB_data[4]

        self.RAW_session_data_len = len( DB_session_data )

        DB_session_data = pickle.loads( DB_session_data )

        return DB_session_id, DB_timestamp, DB_client_IP, DB_client_domain_name, DB_session_data
